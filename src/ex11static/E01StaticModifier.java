...
class MyStatic{
	...
	/*
	 [정적 멤버메소드] : 정적메소드 안에서는 인스턴스 멤버를 사용할 수없다. 정적멤버만 사용이 가능하다.
	...
	public static void main(String[] args) {
		...
		/*
		 방법2 : 인스턴스변수처럼 참조변수를 통해 접근할 수 있다.
		 	 단, 이경우는 정적변수를 인스턴스변수로 착각할 수 있으므로
		 	 권장하지 않는다.
		 */
		...
		System.out.println("ms2.instanceVar="+ms2.instanceVar);//100
		 ...
		 */
		ms1.staticVar = 800; // ms1.staticVar => MyStaic.staticVar
		ms2.staticVar = 900; // ms2.staticVar 로 하게되면 두개의 값이 다른것으로 혼동할 수 있음. => MyStaic.staticVar
		/*
		 위 두개의 문장은 동일한 변수에 접근하고 있으나
		 코드 레벨에서는 마치 다른 변수처럼 보인다.
		 따라서 위 문장은 MyStaic.staticVar 로 
		 기술하는 것이 좋다.
		 */
		...
		//MyStatic.instanceMethod();//에러발생(인스턴스는 참조변수를 통해서만 호출가능)
		MyStatic.staticMethod();
	}
