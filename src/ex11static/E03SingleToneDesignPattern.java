...
/*
 싱글톤 디자인 패턴
 	...
 	2.외부클래스에서 생성자를 접근할 수 없으므로 new연산자를 통해
 	객체생성은 불가능해진다.
 	3.클래스 내부에 정적메소드로 해당 클래스의 객체를 반환하도록
 	정의한다.
 	4.위 방법을 통해 객체를 생성하면 메모리에 하나만 만들어지게된다.
 */
...
class SingleTone{
	int shareVar;
	/*
	 JVM(자바 가상 머신)에 의해 프로그램이 시작될 때 정적변수는 메소드영역에 미리 생성되어 사용할 준비를 마치게된다.
	 */
	private static SingleTone single = new SingleTone();
	/*
	 생성자를 PRIVATE으로 선언하면 클래스외부에서는 접근할 수 없으므로 NEW 연산자를 통해 객체생성은 불가능해진다.
	 */
	private SingleTone() {
		//기본생성자
	}
