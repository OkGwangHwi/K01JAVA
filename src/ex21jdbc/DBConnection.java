...
public class DBConnection {
	...
	public static void main(String[] args) throws ClassNotFoundException,SQLException {
		...
		/*
		 2] 오라클에 연결하기 위한 커넥션URL, 계정ID, 패스워드를 준비한다.
		 	커넥션URL =>
		 		...
		 		※서버환경에 따라 ip주소,포트번호,sid는변경될 수 있다.
		 */
		...
		/*
		 2-1] DriverManager 클래스의 getConnection()메소드를 호출하여
		...
		if(con!=null) {
			...
			/*
			 4] 쿼리문 전송을 위한 Statement계월의 객체를 생성한다.
			...
			/*
			 6] 자원반납 : 모든 작업을 마친후에는 메모리 절약을 위해
			 	연결했던 자원을 반납한다.
			 */
			rs.close();//ResultSet 객체
			stmt.close();//Statement 객체
			con.close();//Connection 객체
		}
		else {
